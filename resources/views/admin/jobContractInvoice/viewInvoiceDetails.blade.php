...
@section('content')
          ...
          <div class="row">
            ...
            <div class="col-md-2">
              {{--@if(!empty(Session::get('sales_add')))--}}
                <a href="{{ url("contract/sales/add") }}" class="btn btn-block btn-default btn-flat btn-border-orange">{{ trans('message.extra_text.new_sales_invoice') }}</a>
              {{--@endif--}}
            </div>
          </div>
        </div>
      ...
      <div class="col-md-8 right-padding-col8">
          <div class="box box-default">
              <div class="box-body">
                  ...
                  @elseif($contractDataInvoice->total <= $contractDataInvoice->paid_amount)
                  ...
                  @endif
                    ...
                    <div class="btn-group pull-right">
                      ...
                      <a target="_blank" href="{{URL::to('/')}}/contract/invoice/print/{{$jobContractData->job_contract_no}}/{{$contractDataInvoice->job_contract_no}}" title="Print" class="btn btn-default btn-flat">{{ trans('message.extra_text.print') }}</a>
                      <a target="_blank" href="{{URL::to('/')}}/contract/invoice/pdf/{{$jobContractData->job_contract_no}}/{{$contractDataInvoice->job_contract_no}}" title="PDF" class="btn btn-default btn-flat">{{ trans('message.extra_text.pdf') }}</a>
                      
                      {{--@if(!empty(Session::get('sales_edit')))--}}
                      <a href="{{URL::to('/')}}/contract/sales/edit/{{$contractDataInvoice->job_contract_no}}" title="Edit" class="btn btn-default btn-flat">{{ trans('message.extra_text.edit') }}</a>
                      {{--@endif--}}

                      {{--@if(!empty(Session::get('sales_delete')))--}}
                      <a href="{{ URL::to('/')}}/contract/invoice/delete-invoice/{{$contractDataInvoice->job_contract_no}}" class="btn btn-default btn-flat delete-btn delete_data" type="button">
                            {{ trans('message.extra_text.delete') }}
                      </a>
                      {{--@endif--}}

                      @if($contractDataInvoice->total > $contractDataInvoice->paid_amount)
                      {{--@if(!empty(Session::get('sales_add')))--}}
            ...
            <div class="box-body">
              <div class="row">
                ...
                <div class="col-md-4">
                  <strong>{{ trans('message.table.invoice_no').' # '.$contractDataInvoice->reference }}</strong>
                  <h5>{{ trans('message.extra_text.location')}} : {{$contractDataInvoice->location_name}}</h5>
                  <h5>{{ trans('message.invoice.invoice_date')}} : {{formatDate($contractDataInvoice->contract_date)}}</h5>
                  ...
                  <div class="box-body no-padding">
                    ...
                    <table class="table table-bordered" id="salesInvoice">
                      ...
                      @if(count($invoiceData)>0)
                       ...
                       <?php
                        $subTotalDiscount = ($subTotal*$contractDataInvoice->discount_percent)/100;
                       ...
                       <tr class="tableInfos"><td colspan="5" align="right">{{ trans('message.table.discount') }}(%)</td><td align="right" colspan="2">{{ $contractDataInvoice->discount_percent }}</td></tr>
                      ...
                      @endif
                      @endforeach
                      <tr class="tableInfos"><td colspan="5" align="right">{{ trans('message.invoice.paid') }}</td><td colspan="2" class="text-right"> {{ Session::get('currency_symbol').number_format($contractDataInvoice->paid_amount,2,'.',',') }}</td></tr>
                      <tr class="tableInfos"><td colspan="5" align="right"><strong>{{ trans('message.invoice.due') }}</strong></td><td colspan="2" class="text-right"><strong>
                        @if(($subTotal+$taxAmount-$contractDataInvoice->paid_amount)<0)
                        -{{ Session::get('currency_symbol').number_format(abs($subTotal+$taxAmount-$contractDataInvoice->paid_amount),2,'.',',') }}
                        @else
                        {{ Session::get('currency_symbol').number_format(abs($subTotal+$taxAmount-$contractDataInvoice->paid_amount),2,'.',',') }}
                        @endif
                      </strong></td></tr>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <p>{{$contractDataInvoice->reference}}</p>
        </div>
        ...
        <form class="form-horizontal" id="payForm" action="{{url('payment/save')}}" method="POST">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-6">
              <input type="number" name="amount" value="{{$contractDataInvoice->total-$contractDataInvoice->paid_amount}}" class="form-control" id="amount" placeholder="Amount">
            </div>
          </div>
          <div class="form-group">
  ...
  <!--Modal start-->
    <div id="emailInvoice" class="modal fade" role="dialog">
      <div class="modal-dialog">
        <form id="sendVoiceInfo" method="POST" action="{{url('contract/invoice/email-invoice-info')}}">
        <input type="hidden" value="{{csrf_token()}}" name="_token" id="token">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            $subjectInfo = str_replace('{invoice_reference_no}', $contractDataInvoice->reference, $subjectInfo);
            ...
            <div class="form-group">
              ...
              <div class="form-groupa">
                  ...
                  $bodyInfo = str_replace('{invoice_reference_no}',$contractDataInvoice->reference, $bodyInfo);
                  ...
                  $bodyInfo = str_replace('{total_amount}', $contractDataInvoice->total, $bodyInfo);
                  ?>
